...
class ArtxPage
{
    ...
    public function positions($positions, $style)
    {
        ...
        if (count($cells) == 0)
            return '';
        $result = '<div class="cpac-content-layout">';
        $result .= '<div class="cpac-content-layout-row">';
        foreach ($cells as $name => $width)
            $result .='<div class="cpac-layout-cell' . ('cpac-block' == $style ? ' cpac-layout-sidebar-bg' : '')
     ...
     *
     * The componentWrapper method gets the content of the 'component' buffer and searches for the '<div class="cpac-post">' string in it.
     * Then it wraps the content of the buffer with the cpac-post.
     */
    public function componentWrapper()
    {
        ...
        if (false === strpos($content, '<div class="cpac-post'))
        ...
        CKEDITOR.on('instanceReady', function (evt) {
            ...
            if (includesTemplateStyle || includesEditorStyle) {
                ...
                if (includesTemplateStyle)
                    jQuery('body', ifrdoc).attr('id', 'cpac-main').addClass('cpac-postcontent');
            }
        });
    }
});
