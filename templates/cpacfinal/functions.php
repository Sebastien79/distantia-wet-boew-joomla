...
if (!defined('_ARTX_FUNCTIONS')) {
    ...
    /**
     * Decorates an article or block with the cpac-post style.
     *
    ...
    function artxPost($data)
    {
        ...
        $classes = isset($data['classes']) && strlen($data['classes']) ? $data['classes'] : '';
                    artxFragmentBegin("<article class=\"cpac-post" . $classes . "\">");
            artxFragmentBegin("<div class=\"cpac-postmetadataheader\">");
            artxFragmentBegin("<h2 class=\"cpac-postheader\">");
            ...
            artxFragmentEnd("</div>");
            artxFragmentBegin("<div class=\"cpac-postheadericons cpac-metadata-icons\">");
            if (isset($data['metadata-header-icons']) && count($data['metadata-header-icons']))
                foreach ($data['metadata-header-icons'] as $icon)
                    artxFragment('', $icon, '', ' | ');
            artxFragmentEnd("</div>");
            artxFragmentBegin("<div class=\"cpac-postcontent cpac-postcontent-0 clearfix\">");
            if (isset($data['content']) && strlen($data['content']))
                artxFragmentContent(artxPostprocessPostContent($data['content']));
            artxFragmentEnd("</div>");
            artxFragmentBegin("<div class=\"cpac-postmetadatafooter\">");
            artxFragmentBegin("<div class=\"cpac-postfootericons cpac-metadata-icons\">");
    ...
    function artxBlock($caption, $content, $classes = '')
    {
        ...
        if (!$hasCaption && !$hasContent)
            return '';
                    artxFragmentBegin("<div class=\"cpac-block clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"cpac-blockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"cpac-blockcontent\">");
    ...
    function artxVMenuBlock($caption, $content, $classes = '')
{
    ...
    if (!$hasCaption && !$hasContent)
        return '';
                artxFragmentBegin("<div class=\"cpac-vmenublock clearfix" . $classes . "\">");
            artxFragmentBegin("<div class=\"cpac-vmenublockheader\"><h3 class=\"t\">");
            artxFragmentContent($caption);
            artxFragmentEnd("</h3></div>");
            artxFragmentBegin("<div class=\"cpac-vmenublockcontent\">");
            artxFragmentContent(artxPostprocessBlockContent($content));
            artxFragmentEnd("</div>");
     ...
     *  Select inputs with class != 'cpac-button'
     *   artxFindTags($html, array('input' => array('class' => '!cpac-button')))
     *  Select inputs with non-empty class
     *   artxFindTags($html, array('input' => array('class' => '!')))
     *  Select inputs with class != 'cpac-button' and non-empty class:
     *   artxFindTags($html, array('input' => array('class' => array('!cpac-button', '!'))))
     *  Select inputs with class = button but != 'cpac-button'
     *   artxFindTags($html, array('input' => array('class' => array('button', '!cpac-button'))))
     *
    ...
    function artxReplaceButtons($content, $filter = array('input' => array('class' => 'button')))
    {
        ...
        foreach (artxFindTags($content, $filter) as $tag) {
            if (is_string($tag))
                ...
                $tag['attributes']['class']['value'][] = 'cpac-button';
    ...
    function artxLinkButton($data = array())
    {
        return '<a class="' . (isset($data['classes']) && isset($data['classes']['a']) ? $data['classes']['a'] . ' ' : '')
            . 'cpac-button" href="' . $data['link'] . '">' . $data['content'] . '</a>';
    }
    ...
    function artxTagBuilder($tag, $attributes = array(), $content = '', $level) {
        if($level==2){
            //$titleTag = '<h4 role="presentation">';
            //$titleEndTag = '</h4>';
        }
    ...
    function artxPostprocessContent($content)
    {
        ...
        $content = artxReplaceButtons($content, array('input' => array('class' => array('button', '!cpac-button')),
            'button' => array('class' => array('button', '!cpac-button')), 'button' => array('class' => array('btn', '!cpac-button'))));
        return $content;
    }
